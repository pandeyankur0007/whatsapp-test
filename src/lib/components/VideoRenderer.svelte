<script lang="ts">
  ...
  import { enableGPUAcceleration, disableGPUAcceleration } from '../utils/performance';
  ...
  onMount(() => {
    ...
    const updateEventName = isLocal ? 'local-video-updated' : 'remote-video-updated';
    ...
    return () => {
      ...
      window.removeEventListener(updateEventName, handleVideoReady as EventListener);
